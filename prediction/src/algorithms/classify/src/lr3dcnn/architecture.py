...
def net():
    ...
    coders = [_encoder(in_tensor=tnsr, stride=stride, bn_axis=self.channel_axis,
                       kernel=kernel, dropout=dropout_conv)
              for tnsr, stride, kernel in zip(inputs, strides, kernels)]
    x = average(coders)

    #   shape:  128, 9, 10, 10
    x = _conv_block(x, 3, [128, 128], dropout=dropout_conv, bn_axis=self.channel_axis)
    x = _identity_block(x, 3, [128, 128], dropout=dropout_conv, bn_axis=self.channel_axis)
    x = _conv_block(x, 3, [256, 256], stride=(2, 2, 2), dropout=dropout_conv, bn_axis=self.channel_axis)
    x = _identity_block(x, 3, [256, 256], dropout=dropout_conv, bn_axis=self.channel_axis)
    x = keras.layers.BatchNormalization(axis=self.channel_axis)(x)
    x = keras.layers.Activation('relu')(x)
